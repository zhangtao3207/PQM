...
assign wave_external_copy_start =
    ...
    !wave_frame_copy_active && !wave_resample_pending;
...
assign wave_frame_copy_start    = wave_external_copy_start || wave_first_frame_copy_start || wave_internal_copy_start;
...
wave_display_resampler #(
    ...
    .VERT_SCALE_NUM(VERT_SCALE_NUM),
    .VERT_SCALE_DEN(VERT_SCALE_DEN),
    .CENTER_DEFAULT(CENTER_DEFAULT)
...
always @(posedge wave_clk or negedge sys_rst_n) begin
    if (!sys_rst_n) begin
        ...
        if (wave_frame_commit_valid) begin
            wave_display_bank <= wave_frame_commit_bank;
            wave_frame_valid  <= 1'b1;
        end

        if (wave_first_frame_copy_start || wave_internal_copy_start) begin
            trigger_pulse        <= 1'b1;
